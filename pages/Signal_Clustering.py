...
with c1:
    p = converter.convert(dendrogram, width=400, height=600, 
        orientation="right  ", scale="symlog", show_nodes=True, key="idendrogram",
        margins={'top': 50, 'bottom': 50, 'left': 50, 'right': 50}
    )

placeholder = c2.container()

if p:
    placeholder.subheader(f"Selected Node ID: {p.id}")
